...
const ReviewItem = memo(({ review, user, handleLike, handleDelete }) => {
  ...
  return (
    <div className="border-b border-border-light py-4">
      <p className="text-text-secondary text-xl flex items-center gap-2">
        <span className="bg-success text-white text-xs px-1 py-0.5 rounded">
          {review.rating}★
        </span>
        {review.comment}
      </p>
      ...
      <div className="flex justify-between items-center mt-2 text-text-muted text-sm">
        <p>
        ...
        <button
          ...
          className={`flex items-center gap-1 ${isLiked ? "text-primary" : "text-text-muted"}`}
        >
      ...
      {isOwner && (
        <button onClick={() => handleDelete(review._id)} className="text-danger text-xs mt-1">
          Delete
        </button>
      )}
    </div>
...
const ReviewSection = ({ productId }) => {
  ...
  if (error) return <p className="text-center text-danger mt-6">{error}</p>;
  ...
  return (
    <div className="mt-12 max-w-3xl mx-auto px-4 min-h-[400px]">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={handleAddReview} className="bg-primary text-white px-4 py-2 rounded">
          Add Review
        </button>
      </div>

      {totalReviews > 0 && (
        <div className="flex gap-10 border-b border-border pb-6 mb-6">
          <div>
              ...
              return (
                <div key={star} className="flex items-center gap-2 text-sm">
                  ...
                  <div className="flex-1 h-2 bg-bg-muted rounded">
                    <div className="h-2 bg-success rounded" style={{ width: `${percent}%` }} />
                  </div>
                  <span>{count}</span>
                </div>
              );
      ...
      {reviews?.length === 0 ? (
        <div className="text-center text-text-muted mt-6">No reviews yet</div>
      ) : (
        reviews.map((review) => (
          <ReviewItem
            key={review._id}
