...
const ChatWidget = () => {
  ...
  return (
    <>
      <button
        onClick={toggleChat}
        className="fixed bottom-6 right-6 bg-pink-600 text-white rounded-full w-16 h-16 flex items-center justify-center shadow-lg hover:bg-pink-700 transition"
      >
        {isOpen ? <FiX size={26} /> : <FiMessageCircle size={26} />}
      </button>

      {isOpen && (
        <div className="fixed bottom-24 right-6 w-[380px] h-[520px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-gray-50 flex flex-col space-y-3">
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="px-4 py-2 rounded-xl max-w-[75%] text-sm shadow bg-white text-black text-xs">
                  Nyra is typing...
                </div>
              </div>
            )}
